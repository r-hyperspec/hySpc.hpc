...
///
/// Stage 2 deliverable: extract `p`, `i`, `x` slots of a `dgCMatrix` on
...
fn dgc_row_sums_rust(
   ...
   // Zero-copy borrows of the R vectors backing the dgCMatrix slots`as_integer_slice` / `as_real_slice` return `None` if the underlying
   // because passing e.g. `dgCMatrix@p` already coerced to double would otherwise break pointer arithmetic in Rust.
   let p_slice = p
       .as_integer_slice()
   ...
   let mat: SparseColMat<usize, f64> = dgcmatrix_to_faer(&view)
       .map_err(|e| Error::Other(format!("dgCMatrix -> faer conversion failed: {}", e)))?;

   // Walk the reconstructed faer matrix's CSC storage so the test actually exercises the round-tripped sparse matrix and not the original R slices.
   let symbolic = mat.symbolic();
   let col_ptrs: &[usize] = symbolic.col_ptrs();
   let row_indices: &[usize] = symbolic.row_indices();
   let values: &[f64] = mat.values();
